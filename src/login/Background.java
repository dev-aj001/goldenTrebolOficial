...
public class Background extends JComponent {
    ...
    public Background() {
        image = new ImageIcon(getClass().getResource("/login/fondo.jpg"));
    }
    ...
    private void createBlurImage(Graphics2D g) {
        ...
        if (width > 0 && height > 0) {
            ...
            Shape shape = new FancyBorderRadius(width, height, "25% 75% 39% 30% / 29% 31% 69% 55% %").getShape();
            g2.fill(shape);
            g2.setComposite(AlphaComposite.SrcIn);
            g2.drawImage(ImageUtil.blur(bufferedImage.getSubimage(x, y, width, height), 30f), 0, 0, null);
            g2.setComposite(AlphaComposite.SrcOver);
            g2.setColor(new Color(15, 15, 15, 60));
            g2.fill(shape);
            g2.dispose();
